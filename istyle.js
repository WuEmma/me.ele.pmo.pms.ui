...
;(function($){
	...
	window.querycpage = {
		...
		channel: function(data){
            ...
            that.c.find('li[data-cid="'+that.cid+'"]').addClass('active')
        ...
        page: function(data){
            ...
            if( that.put ){
                ...
                for (var i = 0; i < data.length; i++) {
                    var item = data[i]
                    h += '<li data-pid="'+item['pageid']+'" data-pname="'+item['pagename']+'" data-url="'+item['url']+'"><small>'+item['pageid']+'</small><b>'+item['pagename']+'</b> ['+item['cname']+']<p>'+item['url']+'</p></li>'
                }
                h += '</ul>'
            }else{
                ...
                if( that.cid != 'init' ){
                    if( that.usechannel && that.cname ){
                        h += '<a style="padding-left:10px;border-bottom: solid 1px #eee;line-height: 42px;display:block;" href="javascript:;" data-pchannel="'+that.cname+'">选择 '+that.cname+' 频道</a>'
                    }
                    h += '<ul class="unstyled">'
                    for (var i = 0; i < m.length; i++) {
	...
	querypage.prototype = {
        ...
        _init: function(el, options){
            ...
            el.bind('click', function(e){
                ...
                that.hem.cid = that.hem.cid || 'init'
                ...
                that.hem.usechannel = that.options.usechannel
                ...
                that._target.on('click',function(e){
                    ...
                    if( that.options.usechannel && _ta.attr('data-pchannel') ){
                        if(typeof that.options.later === 'function') {
                            return that.options.later({
                                channel: that.hem.cname
                            })
                        }
                    }
                    ...
                    if( _ta.attr('data-cid') ){
                        ...
                        if( that.hem.cid === 'init' ){
                            ...
                            that.hem.cname = that.hem.pool.channel[_ta.index()-1].cname
                        }
